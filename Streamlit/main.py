...
def main():
    ...
    st.set_page_config(page_title="Football Tactical Analysis", layout="wide", initial_sidebar_state="expanded")
    ...
    demo_selected = st.sidebar.radio(label="Select Demo Video", options=["Video 1", "Video 2"], horizontal=True)
    ...
    with tab1:
        ...
        st.subheader('Main Application Functionalities:')
        st.markdown("""
    ...
    with tab3:
        ...
        with bcol2:
            ...
            with bcol21t:
                show_k = st.write(label="Show Keypoints Detections", value=False)
                show_p = st.write(label="Show Players Detections", value=True)
            with bcol22t:
                show_pal = st.write(label="Show Color Palettes", value=True)
                show_b = st.write(label="Show Ball Tracks", value=True)
            plot_hyperparams = {
                0: show_k,
                1: show_pal,
                2: show_b,
